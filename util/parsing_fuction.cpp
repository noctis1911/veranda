...
QString parsing_function::hex_to_bin_conversion(QString hex){
    ...
    for (int i = 4; i < hex.size(); i++){
        biner.sprintf("%s%s", biner.toAscii().data(), hex_to_bin(dat[i]).toAscii().data());
    }
...
QString parsing_function::format_5cut_32get(QString biner){
    ...
    for (int i = 0; i < biner.size(); i++){
        if (buang_5bit == false){
            dat_5bit.sprintf("%s%c", dat_5bit.toAscii().data(), dat[i]);
            ...
            dat_32bit.sprintf("%s%c", dat_32bit.toAscii().data(), dat[i]);

            if (c_32bit == 31){
                all_32bit.sprintf("%s%s", all_32bit.toAscii().data(), dat_32bit.toAscii().data());
...
QString parsing_function::parse_data(QString dat, int jum_dat){
    ...
    for (int i = 0; i < dat.size(); i++){
        cnt_p++;
        data.sprintf("%s%c", data.toAscii().data(), dats[i]);
